...
public class CarService implements ICarService {
    ...
    public Page<CarDto> getFilteredCars(CarFilterDto filterDto, Pageable pageable) {
        ...
        if (filterDto.getMileageFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("mileage"), filterDto.getMileageFrom()));
        }
        if (filterDto.getMileageTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("mileage"), filterDto.getMileageTo()));
        }

        // Фильтр по городу
        if (filterDto.getCity() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("city"), filterDto.getCity()));
        }

        // Фильтр по бренду
        if (filterDto.getBrand() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("carModel").get("brand"), filterDto.getBrand()));
        }

        // Фильтр по модели
        if (filterDto.getModel() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("carModel").get("model"), filterDto.getModel()));
        }

        // Фильтр по поколению
        if (filterDto.getGeneration() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("carModel").get("generation"), filterDto.getGeneration()));
        }

        // Фильтр по цене
        if (filterDto.getPriceFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("price"), filterDto.getPriceFrom()));
        }
        if (filterDto.getPriceTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("price"), filterDto.getPriceTo()));
        }

        // Фильтр по количеству владельцев
        if (filterDto.getOwnersCountFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("ownersCount"), filterDto.getOwnersCountFrom()));
        }
        if (filterDto.getOwnersCountTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("ownersCount"), filterDto.getOwnersCountTo()));
        }

        // Фильтр по типу трансмиссии
        if (filterDto.getTransmission() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("transmissionType"), filterDto.getTransmission()));
        }

        // Фильтр по типу кузова
        if (filterDto.getBodyType() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("bodyType"), filterDto.getBodyType()));
        }

        // Фильтр по году выпуска
        if (filterDto.getYearFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("year"), filterDto.getYearFrom()));
        }
        if (filterDto.getYearTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("year"), filterDto.getYearTo()));
        }

        // Фильтр по мощности двигателя
        if (filterDto.getEnginePowerFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("enginePower"), filterDto.getEnginePowerFrom()));
        }
        if (filterDto.getEnginePowerTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("enginePower"), filterDto.getEnginePowerTo()));
        }

        // Фильтр по типу двигателя
        if (filterDto.getEngineType() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("engineType"), filterDto.getEngineType()));
        }

        // Фильтр по приводу
        if (filterDto.getDrive() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("driveType"), filterDto.getDrive()));
        }

        // Фильтр по объему двигателя
        if (filterDto.getEngineCapacityFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("engineCapacity"), filterDto.getEngineCapacityFrom()));
        }
        if (filterDto.getEngineCapacityTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("engineCapacity"), filterDto.getEngineCapacityTo()));
        }

        // Фильтр по положению руля
        if (filterDto.getSteeringPosition() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("steeringPosition"), filterDto.getSteeringPosition()));
        }

        // Фильтр по количеству мест
        if (filterDto.getSeatsFrom() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("seatsCount"), filterDto.getSeatsFrom()));
        }
        if (filterDto.getSeatsTo() != null) {
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.lessThanOrEqualTo(root.get("seatsCount"), filterDto.getSeatsTo()));
        }
